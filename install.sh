...
if ! grep -qi "netname:\s*aeza_international" <<< "$WHOIS_RESULT"; then
  ...
  echo "❌ Ошибка: Скрипт не может быть запущен на данном сервере"
  echo "Текущий провайдер: ${CURRENT_NETNAME}"
  echo " "
  echo "🚀 Зарегистрируйтесь по ссылке: https://aeza.net/?ref=493672"
  echo "🚀 Арендуйте виртуальный сервер"
  echo "✅ Запустите данный скрипт на новом сервере"
  exit 1
fi
echo "✅ Проверка провайдера успешно пройдена"

clear
echo "🌐 Автоматическая установка n8n с сайта n8n-hub.ru"
echo "---------------------------------------------------"
